            ...
            <el-table-column label="属性值操作">
              <template #="{ $index }">
                <el-button type="primary" size="default" icon="Delete"
                  @click="attrParams.attrValueList.splice($index, 1)" v-has="`btn.Trademark.remove`"></el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <el-card v-show="showType === 'sale'" style="margin: 10px 0px">
      ...
      <div v-show="saleScene == 1">
        ...
        <el-button @click="addSaleAttrValue" :disabled="saleAttrParams.attrName ? false : true" type="primary"
          size="default" icon="Plus" v-has="`btn.Trademark.add`">
          添加属性值
        </el-button>
        ...
        <el-table border style="margin: 10px 0" :data="saleAttrParams.attrValueList">
          ...
          <el-table-column label="属性值名称">
            ...
            <template #="{ row, $index }">
              ...
              <div v-else @click="toEditSale(row, $index)">{{ row.valueName }}</div>
            </template>
          </el-table-column>
          <el-table-column label="属性值操作">
            <template #="{ $index }">
              <el-button type="primary" size="default" icon="Delete"
                @click="saleAttrParams.attrValueList.splice($index, 1)" v-has="`btn.Trademark.remove`"></el-button>
            </template>
          </el-table-column>
        </el-table>
...
//引入获取已有属性与属性值接口
import { reqAddOrUpdateAttr, reqAttr, reqSaleAttr, reqRemoveAttr, reqAddOrUpdateSaleAttr } from '@/api/product/attr'
