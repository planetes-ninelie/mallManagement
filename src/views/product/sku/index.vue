...
const edit = async (row: any) => {
  function getSelectAttrValue(isSale: boolean, attrId?: number) {
    const attrValues = isSale ? skuInfo.value.skuSaleAttrValueList : skuInfo.value.skuAttrValueList
  ...
  if (result.code == 200) {
    ...
    const newSkuSaleAttrValueList = skuSaleAttrValueList?.map(({ id, attrId }) => ({
      id, attrId
    }))
    skuInfo.value = {
      ...item,
    ...
    const categoryId = data.skuAttrValueList[0]!.attr!.categoryId as number
    ...
    console.log('at', attrArr.value);

    //获取对应的销售属性
    const spuId = skuInfo.value.spuId || 0
