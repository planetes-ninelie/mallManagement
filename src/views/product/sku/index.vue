        ...
        <el-form :data="skuInfo">
          ...
          <el-form-item>
            ...
            <el-col :span="20">
              ...
              <!-- <img :src="skuInfo.skuImageList[0].imgUrl" height="100%" /> -->
            </el-col>
          </el-form-item>
        </el-form>
      </template>
...
const edit = async (row: any) => {
  ...
  let result2: any = await reqSpuHasSaleAttr(spuId)

  function getSelectAttrValue(isSale, attrId) {
    let attrValues
    if (!isSale) {
      attrValues = skuInfo.value.skuAttrValueList
      const attrValue = attrValues.filter(item => item.attrId === attrId)
      return `${attrValue[0].attrId}:${attrValue[0].id}`
    } else {
      attrValues = skuInfo.value.skuSaleAttrValueList
      const attrValue = attrValues.filter(item => item.attrId === attrId)
      return `${attrValue[0].attrId}:${attrValue[0].id}`
    }
