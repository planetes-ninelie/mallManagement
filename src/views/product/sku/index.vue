<template>
  <el-card>
    <el-table border :data="records" class="table">
      ...
      <el-table-column label="操作" width="300px" fixed="right">
        <template #="{ row }">
          ...
          <el-button icon="Edit" type="primary" @click="edit(row)"></el-button>
          <el-button icon="InfoFilled" type="info" @click="getInfo(row)"></el-button>
    ...
    <el-pagination v-model:current-page="pageNo" v-model:page-size="pageSize" :page-sizes="[3, 5, 10, 15]"
      :background="true" :pager-count="9" layout="prev, pager, next, jumper, ->, sizes, total" :total="total"
    ...
    <el-drawer v-model="drawer">
      ...
      <template #default>
        ...
        <el-row>
          ...
          <el-col :span="20">
            <el-tag type="info" v-for="item in skuInfo.skuAttrValueList" :key="item.id">{{ item.valueName }}</el-tag>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="4">销售属性</el-col>
          <el-col :span="20">
            <el-tag type="info" v-for="item in skuInfo.skuSaleAttrValueList" :key="item.id">{{ item.saleAttrValueName
              }}</el-tag>
          </el-col>
        </el-row>
        <el-row>
...
import { reqCancelSale, reqOnSale, reqSku, reqSkuDelete, reqSkuInfo } from '@/api/product/sku/index'
import { HasSkuResponseData, SkuData, SkuInfoData } from '@/api/product/sku/type';
...
//获取SKU数据
const getSku = async () => {
  let result: HasSkuResponseData = await reqSku(
    pageNo.value,
    pageSize.value,
  )
  if (result.code == 200) {
...
const sale = async (row: any) => {
  let result: any = await ((row.isSale == 1) ? reqCancelSale(row.id) : reqOnSale(row.id))
  if (result.code == 200) {
    ElMessage({
      type: 'success',
